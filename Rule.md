...
12. **파일과 모듈 경계는 한눈에 보이는 책임을 따른다.**
    한 파일에는 하나의 상태·불변식 소유권과 변경 이유를 두고, 공통 메커니즘은 한 공통 owner에, 제품별 의미는 해당 모듈에 둔다. LOC는 설계 기준이 아니라 구조 이상 신호다. 500줄부터 구조를 점검하고, 800줄부터 강한 분리 후보로 보며, 하나의 개념·상태 흐름·불변식으로 강하게 응집된 경우만 유지한다. 1,000줄 초과는 원칙적으로 금지하고, 실제 generated code·fixture·선언적 데이터만 구체 파일과 근거가 생겼을 때 좁은 예외를 검토한다. 크기와 무관하게 서로 다른 도메인 개념, 권한·소유권, 수명주기, 변경 이유, 독립 불변식이 섞이면 즉시 분리한다. 반대로 분리로 상태 전달·공개 인터페이스·모듈 간 의존성·glue code가 늘어 흐름이나 불변식이 더 흐려지면 그 분리를 재검토하고 큰 파일도 유지할 수 있다. 줄 수를 맞추기 위한 분리, 중앙 분기, 순환 의존, forwarding-only wrapper, 중복 변환, provider별 조건이 공통 계층이나 프론트에 누적되는 구조는 완료로 보지 않는다.
...
14. **주기 작업과 재시도는 실패 계약이 있는 소유자만 둔다.**
    Polling, heartbeat, interval, timer, retry는 실제 외부 상태·연결 수명·불확실한 결과를 관찰해야 하는 계약과 구체적 비용 또는 위협이 있을 때만 사용한다. 주기, 상한, 종료 조건, 취소·정리 주체, 실패 노출 방식을 함께 검증한다. 누락된 라우트·이벤트·동기화·provider 통합을 상시 polling이나 fallback으로 덮거나, 실패를 `catch`, `let _`, 빈 값, permissive default로 조용히 삼키지 않는다. 모든 수동 리뷰는 diff뿐 아니라 저장소 전체에서 무의미한 polling, 근거 없는 heartbeat, 소유자 없는 주기 timer, 무한 retry, 실패 은폐가 새 변경과 결합해 활성화되는지도 확인한다.
