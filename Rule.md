...
12. **파일과 모듈 경계는 한눈에 보이는 책임을 따른다.**
    한 파일에는 하나의 권위와 변경 이유를 두고, 공통 메커니즘은 한 공통 owner에, 제품별 의미는 해당 모듈에 둔다. 500줄은 책임 재검토 경고, 800줄은 아주 강한 경고, 1,000줄은 절대 상한이다. 줄 수 한계까지 기다리지 말고 독립적으로 변경·검증되는 책임을 즉시 분리한다. 250줄이어도 서로 다른 개념·권한·변경 이유가 섞이면 쪼개고, 반대로 하나의 개념과 흐름이 선명하며 분리가 불변식을 흐리게 한다면 600~1,000줄도 유지할 수 있다. 작은 파일 수나 추상화 자체를 목표로 억지로 쪼개지 않되, 중앙 분기, 순환 의존, forwarding-only wrapper, 중복 변환, provider별 조건이 공통 계층이나 프론트에 누적되는 구조는 완료로 보지 않는다.
