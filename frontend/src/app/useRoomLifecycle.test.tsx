...
describe("room lifecycle request ownership", () => {
  it("retains exact uncertain intent, suppresses concurrent refresh and retries without retargeting", async () => {
    ...
    act(() => result.current.change(room, "delete", "Wrong"));
    expect(changeRoomLifecycle).toHaveBeenCalledTimes(3);
    vi.mocked(changeRoomLifecycle).mockRejectedValueOnce(new ApiError(503, "pending", "room_deletion_pending", "unresolved"));
    ...
    vi.mocked(changeRoomLifecycle).mockResolvedValueOnce({ room: { room_id: "general", room_uid: "exact-room", label: "General", status: "closed", created_at: "", updated_at: "" }, cleanupPending: false, deleted: true });
    act(() => result.current.retry());
    await waitFor(() => expect(result.current.pending).toBeNull());
