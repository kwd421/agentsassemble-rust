...
async function openReadyHarness(errors: RoomSocketSayError[]) {
  ...
  harness.sockets[0].receive(frames.receipt); harness.sockets[0].receive(frames.catalog); harness.sockets[0].receive(frames.requestsEnd);
...
describe("Agent Session socket contract", () => {
  it("projects an admitted external attendee while rejecting mixed custody and managed creation ACKs", async () => {
    ...
    const snapshot = { ...JSON.parse(frames.rawSnapshot), provider_catalog: frames.catalog.catalog, provider_requests: [] };
