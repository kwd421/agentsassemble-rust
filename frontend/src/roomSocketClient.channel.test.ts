...
describe("custom-channel wire projection", () => {
  ...
  it.each([4, 1])("does not replay a pending channel send into a recreated room at sequence %i", async (newCursor) => {
    ...
    try {
      ...
      sockets[reconnected].receive(next.receipt); sockets[reconnected].receiveRaw(JSON.stringify(next.snap));
      await flushPromises();
      ...
      sockets[reconnected + 1].receive(fresh.receipt); sockets[reconnected + 1].receiveRaw(JSON.stringify(fresh.snap));
      await flushPromises();
  ...
  it("does not accept regressed history or replay pending intent from a resync hint alone", async () => {
    ...
    try {
      ...
      sockets[2].receive(regressed.receipt); sockets[2].receiveRaw(regressed.rawSnapshot);
      await flushPromises();
