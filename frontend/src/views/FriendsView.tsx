...
export default function FriendsView({ onClose }: { onClose: () => void }) {
  ...
  return <section style={{ padding: 24, overflowY: "auto", minHeight: 0, display: "grid", gap: 24, alignContent: "start" }} aria-label="친구 목록">
    <header style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 12 }}>
      ...
      <button type="button" className="ops-button is-primary" style={buttonStyle} disabled={directory.busy || !directory.loaded} onClick={() => setEditing(newFriend())}>친구 추가</button>
    ...
    <div style={{ display: "grid", gap: 12 }}>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        ...
        <button type="button" className="ops-button" style={buttonStyle} aria-pressed={onlineOnly} onClick={() => setOnlineOnly(true)}>온라인</button>
        <label style={{ flex: 1 }}>종류<select className="ops-input" style={inputStyle} value={type} onChange={(event) => setType(event.target.value as typeof type)}>
    ...
    <div style={{ display: "grid", gap: 12 }}>
      {visible.map((friend) => <article key={friend.friend_id} style={{ padding: 20, border: "1px solid var(--color-panel-border)", borderRadius: 12, display: "grid", gap: 12 }}>
        <div style={{ overflowWrap: "anywhere" }}><strong>{friend.details.display_name}</strong>
          <p style={{ margin: "8px 0 0" }}>{[types[friend.details.participant_type], friend.details.provider_kind, friend.details.handle].filter(Boolean).join(" · ")}</p>
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
          <button type="button" className="ops-button" style={buttonStyle} disabled={directory.busy} aria-label={`${friend.details.display_name} 편집`} onClick={() => setEditing({ friend_id: friend.friend_id, expected_revision: friend.revision, details: { ...friend.details } })}>편집</button>
          <button type="button" className="ops-button is-danger" style={buttonStyle} disabled={directory.busy} aria-label={`${friend.details.display_name} 삭제`} onClick={() => setDeleting(friend)}>삭제</button>
        </div>
      </article>)}
    </div>
    ...
    {deleting && <FriendDialog title="친구를 삭제할까요?" busy={directory.busy} onCancel={() => setDeleting(null)}>
      ...
      <div style={{ display: "flex", justifyContent: "end", gap: 12 }}>
        ...
        <button type="button" className="ops-button is-danger" style={buttonStyle} disabled={directory.busy} onClick={async () => { if (await directory.remove(deleting.friend_id)) setDeleting(null); }}>삭제</button>
      </div>
    </FriendDialog>}
  </section>;
}
...
function FriendEditor({ initial, busy, error, onCancel, onSave }: {
  ...
  return <FriendDialog title={initial.expected_revision === 0 ? "친구 추가" : "친구 편집"} busy={busy} onCancel={onCancel}>
    <form style={{ display: "grid", gap: 20 }} onSubmit={(event) => { event.preventDefault(); if (!busy && changed && draft.details.display_name.trim()) void onSave(draft); }}>
      ...
      <label>종류<select className="ops-input" style={inputStyle} value={draft.details.participant_type} disabled={busy} onChange={(event) => setDraft((current) => ({ ...current, details: { ...current.details, participant_type: event.target.value as FriendParticipantType } }))}>
      ...
      <div style={{ display: "flex", justifyContent: "end", gap: 12 }}>
        ...
        <button type="submit" className="ops-button is-primary" style={buttonStyle} disabled={busy || !changed || !draft.details.display_name.trim()}>{busy ? "저장 중…" : "저장"}</button>
      </div>
    </form>
  </FriendDialog>;
}
