...
export default function CompanionInviteCard({ controls }: { controls: CompanionInviteControls }) {
  return <section className="dc-invite-card" style={{ margin: "20px 24px", minWidth: 0 }} aria-label="동반 AI 초대">
    ...
    <label style={{ display: "grid", gap: 8 }}>제공자 이름
      <input className="ops-input" style={{ minHeight: 44, minWidth: 0 }} placeholder="예: codex" value={controls.provider} disabled={controls.creating} onChange={(event) => controls.setProvider(event.target.value)} />
    </label>
    ...
    {controls.invites.map((invite) => <div key={invite.key} className="dc-invite-friend-row" style={{ flexWrap: "wrap" }}>
      <div className="dc-invite-card-copy"><strong>{invite.displayName} · {invite.provider}</strong><p>{invite.copyable ? `만료 ${new Date(invite.expiresAt).toLocaleTimeString()}` : "만료됐거나 주소가 변경된 초대예요."}</p></div>
      <button type="button" className="dc-invite-copy-button" style={{ minHeight: 44 }} disabled={!invite.copyable} onClick={() => void controls.copy(invite.key)}>참가 안내 복사</button>
    </div>)}
  </section>;
}
