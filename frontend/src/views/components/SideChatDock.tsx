...
import { AtSign, MessageSquare, Send, Smile, X } from "lucide-react";
...
export default function SideChatDock({ chat, socket, canPost, mentionables, open, onOpenChange }: {
  open: boolean;
  ...
  useLayoutEffect(() => {
    ...
    if (messages && atBottom.current) messages.scrollTop = messages.scrollHeight;
  }, [open, chat.snapshot?.latest_seq]);
  ...
  return <section aria-label="비공식 사이드챗" style={{ flexShrink: 0, borderTop: "1px solid var(--color-panel-soft)", padding: "0 24px", background: "var(--color-panel-bg)" }}>
    ...
    {open && <dialog ref={dialogRef} aria-label="사이드챗" className="dc-create-channel-modal"
      ...
      <div style={{ display: "grid", minHeight: 64, flexShrink: 0, position: "relative" }}>
      <MentionInput className="ops-input" inputRef={inputRef} value={value} onChange={setValue} disabled={disabled}
