...
export default function RoomManagementModal({ controller }: { controller: ReturnType<typeof useRoomLifecycle> }) {
  ...
  return (
    <div className="dc-modal-backdrop" role="presentation">
      <dialog ref={dialogRef} className="dc-invite-modal" aria-modal="true" aria-label="방 관리"
        ...
        <div className="grid gap-4">
          <div><button type="button" className="ops-button" disabled={controller.busy} onClick={() => void controller.refresh()}>목록 새로고침</button></div>
          ...
          {controller.pending && <button type="button" className="ops-button" disabled={controller.busy} onClick={controller.retry}>같은 요청 다시 확인</button>}
          ...
          {controller.rooms.map((room) => (
            <article className="dc-invite-card" key={room.room_uid}>
              ...
              <div className="flex flex-wrap gap-2">
                {room.status !== "closed" && <>
                  <button type="button" className="ops-button" disabled={locked || Boolean(room.cleanup_pending)} onClick={() => confirm(room, room.status === "archived" ? "restore" : "archive")}>{room.status === "archived" ? "복원" : "보관"}</button>
                  <button type="button" className="ops-button" disabled={locked} onClick={() => confirm(room, "close")}>방 종료</button>
                </>}
                <button type="button" className="ops-button" disabled={locked || Boolean(room.deletion_pending)} onClick={() => confirm(room, "delete")}>방 삭제</button>
              </div>
            </article>
          ))}
          {confirmation && <section className="dc-invite-card" aria-label="방 상태 변경 확인">
            ...
            <div className="dc-create-channel-actions">
              <button type="button" className="ops-button" disabled={controller.busy} onClick={() => setConfirmation(null)}>취소</button>
              <button type="button" className="ops-cta" disabled={locked || (confirmation.action === "delete" && confirmationName !== confirmation.room.label)} onClick={() => {
