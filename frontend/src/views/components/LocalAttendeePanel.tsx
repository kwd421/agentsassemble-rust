...
function LocalCreation({ packet }: { packet: AttendeeEntryPacket }) {
  ...
  const read = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      try {
        ...
        if (!signal?.aborted) { setOperation(status); setEditable(false); }
      } catch (failure) {
      ...
      if (!signal?.aborted) setError(failure instanceof Error ? failure.message : "참가 상태를 확인하지 못했어요.");
  ...
  async function create(request?: FrontendLiveAgentCreateRequest) {
    ...
    try {
      ...
      if (active.current) setOperation(result);
    } catch (failure) {
      // Only an explicit rejection plus the owner's missing record permits editing again.
      if (failure instanceof ApiError && failure.status === 409) {
        try { const status = await fetchLocalAttendee(packet); if (active.current) setOperation(status); }
        catch (readFailure) {
          if (readFailure instanceof ApiError && readFailure.code === "local_attendee_missing") {
            submitted.current = null;
            if (active.current) setEditable(true);
          }
        }
      }
      if (active.current) setError(failure instanceof Error ? failure.message : "참가 결과를 확인하지 못했어요.");
      throw failure;
  ...
  async function command(action: LocalAttendeeAction) {
    ...
    setBusy(true);
    setError("");
    try { const result = await commandLocalAttendee(packet, action); if (active.current) setOperation(result); }
    catch (failure) { if (active.current) setError(failure instanceof Error ? failure.message : "참가 상태를 확인하지 못했어요."); }
    finally { if (active.current) setBusy(false); }
  }
  ...
  return <main style={{ padding: 24, maxWidth: 680, margin: "0 auto", display: "grid", gap: 20 }}>
    ...
    <p role="status">{busy ? "참가 상태를 확인하고 있어요." : operation
