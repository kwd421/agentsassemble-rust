...
export function channelMessageFieldsAreValid(event: RoomEvent): boolean {
  assertExactKeys(event, ["v", "id", "seq", "created_at", "room_id", "type", "actor", "participant_id", "participant_type", "actor_id", "actor_type", "display_name", "content", "message_kind", "channel_id"], "channel message");
  return event.type === CHANNEL_MESSAGE_EVENT_TYPE && isCustomChannelId(event.channel_id) &&
    ...
    event.message_kind === "message" && typeof event.content === "string" && Boolean(event.content) &&
