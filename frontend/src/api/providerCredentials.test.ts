...
describe("provider credential HTTP authority", () => {
  ...
  it("rejects unimplemented providers before issuing authority", async () => {
    ...
    await expect(
      setProviderCredential("ollama", "sentinel-provider-value")
    ).rejects.toThrow("Unsupported API credential provider: ollama");
    await expect(deleteProviderCredential("ollama")).rejects.toThrow(
      "Unsupported API credential provider: ollama"
    );
  ...
  it("routes OpenRouter through its isolated credential account", async () => {
    const invoke = vi
      .fn()
      .mockResolvedValueOnce(HOST_SURFACE)
      .mockResolvedValueOnce(ticket("9"));
    Object.assign(window, { __TAURI_INTERNALS__: { invoke } });
    const fetchMock = vi.fn().mockResolvedValueOnce(statusResponse("missing"));
    vi.stubGlobal("fetch", fetchMock);

    await requestDesktopHostProductSurface();
    await expect(fetchProviderCredentialStatus("openrouter")).resolves.toEqual({
      configured: false,
      source: "missing",
    });
    expect(fetchMock).toHaveBeenCalledWith(
      `${HTTP_BASE_URL}/api/provider-credentials/openrouter`,
      expect.any(Object)
    );
  });

  it("routes Vercel AI Gateway through its isolated credential account", async () => {
    const invoke = vi
      .fn()
      .mockResolvedValueOnce(HOST_SURFACE)
      .mockResolvedValueOnce(ticket("8"));
    Object.assign(window, { __TAURI_INTERNALS__: { invoke } });
    const fetchMock = vi.fn().mockResolvedValueOnce(statusResponse("missing"));
    vi.stubGlobal("fetch", fetchMock);

    await requestDesktopHostProductSurface();
    await expect(fetchProviderCredentialStatus("vercel")).resolves.toEqual({
      configured: false,
      source: "missing",
    });
    expect(fetchMock).toHaveBeenCalledWith(
  ...
  it("routes TokenRouter through its isolated credential account", async () => {
    const invoke = vi
      .fn()
      .mockResolvedValueOnce(HOST_SURFACE)
      .mockResolvedValueOnce(ticket("6"));
    Object.assign(window, { __TAURI_INTERNALS__: { invoke } });
    const fetchMock = vi.fn().mockResolvedValueOnce(statusResponse("missing"));
    vi.stubGlobal("fetch", fetchMock);

    await requestDesktopHostProductSurface();
    await expect(fetchProviderCredentialStatus("tokenrouter")).resolves.toEqual({
      configured: false,
      source: "missing",
    });
    expect(fetchMock).toHaveBeenCalledWith(
      `${HTTP_BASE_URL}/api/provider-credentials/tokenrouter`,
      expect.any(Object)
    );
  });

  it("routes Custom API through its isolated credential account", async () => {
    const invoke = vi
      .fn()
      .mockResolvedValueOnce(HOST_SURFACE)
    ...
    await requestDesktopHostProductSurface();
    await expect(fetchProviderCredentialStatus("custom_api")).resolves.toEqual({
      configured: false,
      source: "missing",
    });
    expect(fetchMock).toHaveBeenCalledWith(
      `${HTTP_BASE_URL}/api/provider-credentials/custom_api`,
      expect.any(Object)
    );
  });
