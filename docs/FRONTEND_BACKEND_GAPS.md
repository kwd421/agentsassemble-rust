...
approved by both manual reviewers; packaged local/remote/restart evidence remains
pending because the verification Mac is locked.
...
| Attachments, personas, pins, and search | General-message attachments, persona list/import/thumbnail, and room search/context remain incomplete. Lobby message-pin list/mutation/navigation is source-connected through purpose-bound local and remote HTTP authority and manually approved, but packaged local/remote/read-only/restart verification remains pending; custom-channel pins remain visibly unavailable until their message owner exists. Profile-avatar upload/read is implemented. Room-appearance persistence, upload/read HTTP, atomic bind/replacement/clear, cleanup, local typed issuance, remote-session bound-read exchange, copied-frontend upload/read, object-URL lifetime, and restart recovery are implemented and packaged-verified. |
