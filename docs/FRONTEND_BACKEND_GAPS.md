...
| Human profile/avatar and room appearance | implemented; `fbebad6` restores Agent Session identity in the shared canonical timeline/history/search profile map, while roster projection and the incomplete Agent identity editor remain reopened |
...
| Agent Session create/configure/lifecycle | partial; `fbebad6` corrects the shared message/search identity projection, while provider completion, cleanup, roster SSoT, and a complete Agent profile mutation owner remain reopened |
...
| Roster, friends, and channels | Strict participant-role control, copied participant-mute control, canonical event projection, and the exact Rust provider-interrupt owner are cut over and packaged-verified. `fbebad6` makes the shared lobby timeline/history/search profile map prefer Agent Session identity and room-owned role, but roster entries still prefer participant identity and remain open. Room friends, room channels, voice presence, and side chat remain incomplete; their active copied entry paths are gated through `f687667`, `eecdb20`, and `d058af8` rather than backed by dummy routes or local state. |
