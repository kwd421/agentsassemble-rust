...
| --- | --- |
| Core room, general messages/history/search/pins/attachments/votes | finite bounded-JSON sync/replay and lobby search implemented; room-wide search overclaim remains reopened; four false capabilities and their copied room-delete, participant-kick, provider-response, and agent-readd controls plus producerless wire/callback state are removed through `cccf513`; the stale re-add guidance is corrected through `d874d92` and both manual reviewers approved HEAD `7b2168f`; frame-proof decision closed by D-02 removal |
| Human profile/avatar and room appearance | implemented for humans and rooms; Agent identity now comes from the Agent Session in timeline/history/search, desktop/mobile rosters, mentions, and typing/progress labels through `fbebad6`, `fe568bb`, `e0a681d`, and `97816b5`; `e869f42` reuses the canonical provider logo for avatarless Agent search results and `0363622` prevents the custom-avatar selector from restyling its nested image; post-correction packaged provider-logo geometry remains explicitly unverified after real provider/catalog blockers; the unsupported Agent identity editor is unavailable through `11d3528` until a distinct complete mutation/asset owner exists |
...
| Agent Session create/configure/lifecycle | partial; `fbebad6`, `fe568bb`, `e0a681d`, `97816b5`, `e869f42`, and `0363622` correct the active identity projections and avatarless search-logo CSS boundary, and `11d3528` removes the unsupported profile mutation UI; the earlier isolated packaged restart plus actual Antigravity/OpenCode turns passed, while Codex start exposed `runtime_start_recovered_gone`; the post-CSS-correction Antigravity turn failed on an unapproved terminal command and the current OpenCode catalog omitted Muse Spark, so corrected visual geometry, provider completion, that lifecycle failure, and a complete Agent profile mutation owner remain open |
| Friends | server owner absent; the normal-startup and room-invite entry paths are inactive through `f687667`; `daadd8d`/`9ee4952` remove the unreachable presentation and `/api/room-friends` client/hook source, `adf17ee`/`2f439f1` remove its directory/activity/profile/DM CSS, and `f16bc2c` removes the remaining HomeSidebar-only CSS; future Phase 5 product completion remains open |
| Side chat | server owner absent; active room, mobile, and RimWorld entry paths are inactive through `eecdb20`, and `a2b2f41` removes the leftover mobile-mode and producerless socket-callback state; `b723715`/`1521067`/`8cd8628` remove the unreachable presentation, browser state, and absent HTTP client contract, and `84ceb13` removes its isolated CSS; future Phase 6 product completion remains open |
| Custom text channels | server message owner absent; create/select/render entry paths are inactive through `d058af8`, so the copied poller is outside the production graph; `a2b2f41` restricts active search routing to `all`/`lobby`; isolated packaged verification on 2026-09-02 confirmed that the current room exposes only fixed `general` and no channel-add control |
...
| Provider operations | four providers registered; only DeepSeek credential operations exist; copied provider-response write UI/state and the always-empty provider-request snapshot field are removed through `cccf513`, and `fbb952f` removes an unimported Python-mirrored permission helper. `582a02e` projects credential availability from the Rust registration owner, so Codex/Antigravity/OpenCode expose no false login or credential controls while DeepSeek retains keyring operations. `edfb7c5` and `c890a9a` remove absent catalog-refresh and provider-usage requests/state; usage is explicitly unsupported. `5b94ac6` and `1959a08` require an exact advertised preferred/default model or explicit user selection instead of substituting the first candidate, and `d081761` removes the producerless `stale_cache` source/status/test while retaining `discovered` and `static_manifest`. Other provider completion and cleanup work remains separate. |
...
| Runtime/update/resources/release diagnostics | the unmounted missing-route runtime-version poller is absent through `d45afb5`; other dormant or partly unreachable copied diagnostics have no parity claim |
...
are removed. Manager-invite create/revoke now verifies the exact captured
server/lineage/room-ID/room-UID tuple at private-control ticket issuance and again in
the persistence mutation transaction. The strict C1b frontend API owns exact
create/revoke dispatch classification and response acceptance. The copied controller
now captures current directory authority, retains each accepted custody record, and
presents only its current safe projection; C2 controller/UI custody and packaged
...
| Admission and invites | Durable human invite/session create, join, verification, expiry, revoke, leave, restart recovery, direct bounded-header person-profile and preference authorization, and the authenticated human WebSocket are implemented at their current persistence/server boundaries. Exact `/join`, `/join/`, `/pair`, and `/pair/` production entrances resolve their actual production assets; exact preflight/join response contracts bind request, room, client, server lineage, and product surface before bearer exposure. The browser's single bounded admission-intent owner now preserves pending retry custody and retires it only through a verified settled write or verified direct removal; a settled marker surviving best-effort deletion remains cleanup-only across reload, RoomGuestSession local-storage expiry/clearing, and later invite navigation. Direct external deletion of the admission-intent session-storage key is outside that guarantee. Real isolated browsers pass one-use/reusable normal and read-only admission, avatar transfer, profile edit, token removal, reload, posting/denial, consumed-link rejection, same-browser reusable recovery, server-restart reconnect, direct preference read/write or read-only denial, and exact leave with durable session revocation. Configured-manual trust, managed quick-tunnel/stable-entry custody, the private no-store status/start/stop controls, backend manager invite create/revoke with exact server/lineage/room-generation tickets and same-transaction revalidation, and the native desktop create/revoke ticket bridge with its exact registered commands, capabilities, and permissions are implemented and tested. The copied modal/controller B2 ingress controls use fresh server-operator tickets, strict status parsing, and generation-owned polling, and their legacy mutable public-URL path is removed. The strict C1b API and C2 controller own exact create/revoke dispatch, current-directory authority, retained custody, safe presentation, UI cutover, and packaged verification. Host claim, companion admission, and operator pairing redeem remain incomplete. |
| Roster, friends, and channels | Strict participant-role control, copied participant-mute control, canonical event projection, and the exact Rust provider-interrupt owner are cut over and packaged-verified. `fbebad6` and `fe568bb` make the shared lobby timeline/history/search profile map and desktop/mobile roster prefer Agent Session identity while preserving room-owned role/mute/membership; mentions and typing follow the same owner through `e0a681d` and `97816b5`, avatarless search results reuse the provider-logo owner through `e869f42`, and `0363622` isolates its nested-image layout from the custom-avatar rule. Room friends, room channels, voice presence, and side chat remain incomplete; their active copied entry paths are gated through `f687667`, `eecdb20`, and `d058af8` rather than backed by dummy routes or local state. |
| Attachments, personas, pins, and search | Persona list/import/thumbnail, exact Agent Session selection, durable bounded prompt application, inert executable-card features, and their final critical-web/Daybreaker approval are complete. The copied packaged flow imports CCv3 JSON/PNG, CHARX, and Risu, renders verified thumbnails, persists the library and selection across restart, replaces/clears/reapplies the selection, and completes one official DeepSeek Flash turn whose final reflects both the selected persona and matching literal lore. Lobby search/context has one canonical Rust persistence owner, exact one-use local tickets, direct remote-session target authorization, and a strict copied-frontend path with no visible-timeline fallback. Packaged local and admitted read-only search/context/navigation survive restart or reload; invite revocation rejects later admission without rewriting an existing session's separate authority. Actual Codex Terra and Antigravity Flash sessions returned withheld adjacent values through the provider-neutral RoomPortal tools. The configured OpenCode Hy3-free path remained explicitly unavailable after an external provider error and produced no mock or fallback result. Concrete custom-channel search/context stays explicitly unavailable until those messages have a Rust authority. General lobby-message attachments are complete through exact one-use local tickets and direct remote-session upload/read authorization, atomic canonical-message binding, copied-frontend lazy rendering/download, Agent Session Room Portal reads, restart recovery, packaged Codex/OpenCode/Antigravity verification, and final critical-web/Daybreaker approval with one explicit accepted non-blocking process-wide occupancy Medium; custom-channel attachments remain unavailable until their message owner exists. Lobby message-pin list/mutation/navigation is complete and packaged-verified; custom-channel pins remain visibly unavailable. Profile-avatar upload/read and room-appearance custody are implemented. |
| Provider settings and diagnostics | DeepSeek credential status/set/delete and its registered read-only ordinary-turn backend are connected and source-tested. Packaged credential presence, provider selection/start/stop, and one official DeepSeek Flash turn are verified; packaged credential set/delete mutation controls were not exercised. The F-07 package confirms only DeepSeek exposes the implemented keyring surface, while Codex/Antigravity/OpenCode expose no login or credential controls. Unimplemented login, catalog-refresh HTTP, and provider-usage operations are no longer exposed or requested through `582a02e`, `edfb7c5`, and `c890a9a`; `1313aba` removes their producerless dynamic quota presentation/visibility contract while retaining the truthful static unsupported notice. Local resources, release health, and runtime version remain missing diagnostics. The unmounted `FrontendUpdateNotice.tsx` and its absent-route poll are removed through `d45afb5`; no runtime-version route or replacement timer is claimed. The original `/api/local/workspace-picker` HTTP route is absent, but packaged desktop creation uses the native Tauri directory picker instead. |
...
| Canonical room commands | Canonical lobby message edit/delete is connected through the copied controls and one Rust room-mutation owner. Packaged local and admitted read/write humans preserve edited state and tombstones through live delivery, reload, and normal restart, while an admitted read-only human exposes neither writable composer controls nor mutation actions; both manual reviewers approved pushed HEAD `23c9f35` at C0/H0/M0/L0. Idle-only `agent.pause` and its state-preserving paused `agent.resume` are connected through the copied controls and packaged-verified with Codex Terra, Antigravity Flash, and OpenCode Muse Spark; paused direct mentions remain durably queued without provider work and resume through the existing floor owner without replacing the resident process or provider conversation. Re-add, busy-turn interrupt, participant kick, room lifecycle, and provider request resolution remain incomplete; their copied agent-readd, participant-kick, room-delete, and provider-response controls are absent through `cccf513`; the stale re-add guidance is corrected through `d874d92` and both manual reviewers approved HEAD `7b2168f`. Canonical lobby history and vote creation/state/summary are connected and packaged-verified. Settings and random operations are implemented at the backend boundary; direct human random controls are intentionally absent because the original React client has none. Strict `participant.role.update`, stopped-session `agent.resume`, `agent.configure`, verified `participant.mute`, and exact self `participant.leave` are connected to their existing copied controls. During packaged OpenCode verification, the free provider failed externally after the UI had entered `응답 중`; the copied interrupt control reported that `agent.interrupt` was not present in the bound server product surface, and the UI did not expose the terminal provider error before explicit stop. This remains an explicit interrupt/error exposure gap, not a search fallback. |
...
- `/api/room-friends` has no Rust route; through `f687667`, normal startup enters the
  implemented lobby and no active Friends or room-invite control calls it.
...
create stopped, create-and-start, stop, resume/restart, reconnect, and the provider
reply. Re-add is not a current copied-control exit and remains deferred to its
complete participant/session transition. Unsupported controls remain explicit
unavailable/error states, not fake data or no-ops.
