...
| Provider settings and diagnostics | DeepSeek credential status/set/delete and its registered read-only ordinary-turn backend are connected and source-tested. Packaged credential presence, provider selection/start/stop, and one official DeepSeek Flash turn are verified; packaged credential set/delete mutation controls were not exercised. The F-07 package confirms only DeepSeek exposes the implemented keyring surface, while Codex/Antigravity/OpenCode expose no login or credential controls. Unimplemented login, catalog-refresh HTTP, and provider-usage operations are no longer exposed or requested through `582a02e`, `edfb7c5`, and `c890a9a`; usage remains visibly unsupported. Local resources, release health, and runtime version remain missing diagnostics. The unmounted `FrontendUpdateNotice.tsx` and its absent-route poll are removed through `d45afb5`; no runtime-version route or replacement timer is claimed. The original `/api/local/workspace-picker` HTTP route is absent, but packaged desktop creation uses the native Tauri directory picker instead. |
