...
| Friends | server owner absent; the normal-startup and room-invite entry paths are inactive through `f687667`; `daadd8d`/`9ee4952` remove the unreachable presentation and `/api/room-friends` client/hook source, while `adf17ee`/`2f439f1` remove its directory/activity/profile/DM CSS; separate dormant HomeSidebar CSS and future Phase 5 product completion remain open |
...
| Roster, friends, and channels | The active-room roster, strict participant-role control, copied participant-mute control, canonical event projection, and exact Rust provider-interrupt owner are cut over and packaged-verified. Room friends, room channels, voice presence, and side chat remain incomplete; their active copied entry paths are gated through `f687667`, `eecdb20`, and `d058af8` rather than backed by dummy routes or local state. Dormant-source cleanup and cross-review remain open. |
