...
| Agent Session create/configure/lifecycle | fourteen managed providers pass current local Phase 1 verification and await whole-phase review closure; Phase 2 owns missing re-add and exact control advertisement, while Phase 3 now implements the separate Agent profile mutation/asset boundary. Earlier native-provider failures remain revision-bounded historical evidence; final actual-provider verification follows the current plan. |
...
| Provider operations | fourteen managed providers share the authoritative registry, native completion/cleanup contracts, and credential capability projection. Freebuff is excluded and managed Antigravity is retired; external Antigravity belongs only to Phase 7 Room Connector admission. Local Phase 1 evidence and review corrections are in `docs/VERIFICATION.md`; actual-provider verification is deferred to the plan's final matrix. Unsupported provider usage and permission-response surfaces remain absent. |
