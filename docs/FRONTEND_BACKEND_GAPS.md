...
| Provider operations | four providers registered; only DeepSeek credential operations exist; copied provider-response write UI/state and the always-empty provider-request snapshot field are locally removed through `cccf513`, and `fbb952f` removes an unimported Python-mirrored permission helper, while other login/usage/refresh controls overclaim |
...
| Provider settings and diagnostics | DeepSeek credential status/set/delete and its registered read-only ordinary-turn backend are connected and source-tested. Packaged credential presence, provider selection/start/stop, and one official DeepSeek Flash turn are verified; packaged credential set/delete mutation controls were not exercised. Other missing diagnostics are login, catalog refresh HTTP response, provider usage, local resources, release health, and runtime version. The unmounted `FrontendUpdateNotice.tsx` and its absent-route poll are removed through `d45afb5`; no runtime-version route or replacement timer is claimed. The original `/api/local/workspace-picker` HTTP route is absent, but packaged desktop creation uses the native Tauri directory picker instead. |
