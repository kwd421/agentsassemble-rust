...
| Provider operations | four providers registered; only DeepSeek credential operations exist; copied provider-response write UI/state and the always-empty provider-request snapshot field are removed through `cccf513`, and `fbb952f` removes an unimported Python-mirrored permission helper. `582a02e` projects credential availability from the Rust registration owner, so Codex/Antigravity/OpenCode expose no false login or credential controls while DeepSeek retains keyring operations. `edfb7c5` and `c890a9a` remove absent catalog-refresh and provider-usage requests/state; usage is explicitly unsupported. Exact preferred-model selection and the first-discovered-model substitution remain open. |
