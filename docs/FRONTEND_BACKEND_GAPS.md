...
| Human profile/avatar and room appearance | implemented for humans and rooms; Agent identity now comes from the Agent Session in timeline/history/search, desktop/mobile rosters, mentions, and typing/progress labels through `fbebad6`, `fe568bb`, `e0a681d`, and `97816b5`; `e869f42` reuses the canonical provider logo for avatarless Agent search results; the unsupported Agent identity editor is unavailable through `11d3528` until a distinct complete mutation/asset owner exists |
...
| Agent Session create/configure/lifecycle | partial; `fbebad6`, `fe568bb`, `e0a681d`, `97816b5`, and `e869f42` correct the active identity projections and avatarless search logo, and `11d3528` removes the unsupported profile mutation UI; isolated packaged restart plus actual Antigravity/OpenCode turns pass, while Codex start exposes `runtime_start_recovered_gone`; provider completion, that lifecycle failure, cleanup, and a complete Agent profile mutation owner remain open |
