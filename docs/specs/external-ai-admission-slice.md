...
exact final HEAD and whole local contract at C0/H0/M0/L0;
[disposition and verification](../VERIFICATION.md#phase-7-whole-phase-review-correction-muted-owner-2026-09-09)
...
Driver and launch errors now support owned serialized code/message values; native
constants remain borrowed. Persistence and response owners preserve those codes
without a static-string restriction. Adapter messages use `Box<str>` to keep the
error below the existing large-error lint threshold; allocation occurs on error,
with no new normal-path task, queue or persistence. Existing status classifications
and retry/uncertainty meanings are unchanged. The 321 persistence, 101 server unit,
