...
- Start and stop effects begin only after a `prepared` intent commits. A successful start reports process reuse and provider-conversation reuse separately, and `provider_session_active` comes from the observed provider thread rather than process presence. Confirmed stop is checkpointed as `effect_applied` before finalization; an ambiguous stop instead commits a redacted `disconnected` state with recovery required and never claims success.
- Before the server admits HTTP or WebSocket traffic, every non-closed room session left in `starting`, `idle`, `busy`, `paused`, `recovering`, or `stopping` is detached and marked `disconnected` unless a future startup owner can prove and adopt its exact live handle. Provider conversation identity is retained for explicit resume, while stale process handles and lifecycle intents are cleared.
