...
the Codex/OpenCode inference paths described later are implementation history, not
approved target contracts. Findings F-01 through F-03 in
`docs/architecture/REPOSITORY_AUDIT_2026-09-01.md` own their correction. The slice
also remains incomplete until Agent Session identity is its own display/avatar
SSoT and every exposed provider operation has a real server owner. No earlier
review verdict closes these reopened boundaries.
...
- Verification must prove exact replay/conflict and invalid-state rejection, no assignment while
  ...
  and restart, copied-control surface gating, and the final user-authorized process-preserving
  flows in `docs/PRODUCT_REIMPLEMENTATION_PLAN.md`. Resource evidence compares the resident process set
...
Historical interrupt investigation (superseded by the 2026-09-07 managed-provider
retirement below): critical review found that Antigravity's prior Ctrl-C path stopped the runtime after accepting the
...
At that revision Antigravity parity remained incomplete without an exact native
...
- Codex lifecycle start binds its official executable and code-mode companion as one byte-identified bundle. The guardian's stopped launcher starts the companion as a same-anchor-group child on one validated canonical IPv4-loopback HTTP endpoint for its native gRPC transport, passes only that endpoint through the official `--code-mode-host` option, and then execs the persistent `app-server --stdio`. The companion receives a sanitized environment and runtime custody token but no RoomPortal bearer; unexpected inherited protocol descriptors are replaced with `/dev/null`. Missing or changed companion authority, invalid readiness, early exit, or custody change fails the launch without a fallback.
...
## Historical first-bundle boundaries (superseded by the current phase plan)
...
The original first-bundle sequencing below is historical. Current managed-provider
coverage, phase acceptance, and final actual-provider verification are owned by
`docs/PRODUCT_REIMPLEMENTATION_PLAN.md`; retired providers cannot be selected or
launched through this contract.
...
2. The React room projects the retained managed catalog, exact discovered model/control values, and a nonempty catalog revision. Retired stored selections fail visibly without substitution.
...
2. The final user-authorized real-client matrix in `docs/PRODUCT_REIMPLEMENTATION_PLAN.md` passes and is recorded in `docs/VERIFICATION.md`. External Antigravity is verified only through its Phase 7 Room Connector admission. Missing availability remains failed or unknown and never substitutes a model.
