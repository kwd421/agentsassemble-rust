...
오늘 가위바위보 구간에서 하네스별 차례당 입력 토큰을 쟀다(각 프로그램이 남긴 사용량 기록 기준).
Codex(Terra) 약 6.3만, Sonnet 5 약 14만, grok 약 24만. 셋 다 대부분 캐시였다. 대화 기록이
쌓여 매번 다시 보내지는 것은 모든 에이전트가 그렇게 동작하는 정상이고, 커서 설계상 일부러
세션을 이어 쓰는 것이다. 낭비로 본 것은 두 가지였고 둘 다 고쳤다.

- **방 읽기 결과가 너무 컸다.** 검색 결과마다 ID 두 개·긴 타임스탬프·메시지 본문 전체가
  들어갔고, 문맥 읽기는 `RoomEvent` 를 통째로 돌려줘 한 번에 16,000 자가 나온 적이 있다. 이
  결과는 하네스 기록에 남아 이후 모든 호출에 다시 실린다. 이제 `#seq 작성자: 본문` 한 줄씩으로
  보내고, 검색은 300 자 미리보기, 문맥은 메시지당 1,200 자로 자른다. 후속 읽기에 필요한 event ID
  는 남긴다.
- **종결 툴 뒤의 마지막 한 줄.** 하네스는 툴 결과를 받으면 모델을 한 번 더 부른다. 발행·넘김
  뒤에 쓰는 마무리 문장은 방에 올라가지 않는데, 그 한 번에 grok 기준 약 6.8만 토큰이 다시 나갔다.
  API 드라이버(`remote_openai`)는 원래 종결 툴에서 바로 끝냈다. 포털이 종결 동작이 등록되는
  순간을 알리게 하고, ACP 클라이언트(grok · Cursor)는 그때 턴을 취소하고 정상 종료로 처리한다.
  실제 grok 턴에서 읽기 → 발행 → 취소로 끝나 세 번째 호출이 없어졌고, 메시지는 바로 올라갔으며
  세션은 대기로 돌아왔다.

클로드와 Codex 는 아직 하지 않았다. Codex 는 `turn/interrupt` 로 같은 처리를 구현했으나 가짜
app-server 경계 테스트와 실제 턴을 돌리지 못해 되돌렸다. 알림을 읽는 도중에 끊으면 스트림이
깨지므로 알림 사이에서 확인하는 방식이어야 한다. 클로드는 종결 툴 뒤에 멈추게 하는 훅이 필요하고,
결과 메시지 검증(`validResult`)과 맞는지 실제 턴으로 확인해야 한다.

**열린 문제.** 조기 취소한 grok 턴에서 grok 로그에 `cancel.processing` 만 남고 완료 기록이
없었다. 이어서 grok 이 켜진 채로 앱을 닫자 종료에 약 10 분 30 초가 걸렸다. 종료 유예 한도는
683 초(서버 680 초 + 3 초)이고, 이를 거의 다 쓴 것은 무언가가 정상적으로 끝나지 않고 한도에
걸렸다는 뜻이다. 조기 취소가 grok 을 걸린 상태로 남겨 종료가 기다린 것인지는 확인하지 못했다.
같은 grok 세션으로 두 번째 턴을 돌려 보는 것, 조기 취소 전후로 종료 시간을 비교하는 것이 다음
확인이다. 그전까지는 앱을 닫기 전에 에이전트를 중지해 두는 편이 빠르다.
