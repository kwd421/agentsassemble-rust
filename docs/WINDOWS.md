...
**앱 종료가 서버까지 안 가던 문제 — 원인은 닫는 방법(`taskkill`)으로 본다.** 2026-09-21 grok 이 켜진
상태에서, 2026-09-22 에이전트를 모두 멈춘 상태에서 같은 증상이 났다: 창은 사라졌는데 데스크톱 ·
감독(`agentsassemble-runtime-supervisor`) · 서버 프로세스가 남고, `/healthz` 는 200, `runtime.sqlite3`
는 앱 시작 직후 뒤로 수정되지 않았다(서버가 종료를 시작하지 않음). 두 번 다 디버그 빌드의 콘솔 창을
닫자 데스크톱이 끝나고 서버가 정상 종료했다. 그러니 grok 이나 조기 취소와는 무관하다.

경로: 데스크톱은 Tauri `RunEvent::Exit` 에서만 `LocalRuntime::stop` 을 불러 감독의 stdin 을 닫고,
감독은 그 EOF 를 서버 stdin 으로 넘겨 서버를 끈다. 2026-09-22 재현에서 첫 `taskkill /PID <desktop>`
(강제 아님) 뒤 데스크톱의 메인 창은 파괴되지 않고 숨겨져 있었다 — 두 번째 인스턴스를 띄우자
single-instance 플러그인이 그 창을 다시 보여줬다. 즉 `Exit` 에 도달하지 않았다. 데스크톱 밑에
감독 · 서버 외 자식은 없었고(파이프를 물려받은 프로세스 없음), 콘솔 선택 모드도 아니었다(Esc 무효,
서버 정상 응답). `taskkill` 은 프로세스의 **모든** 최상위 창(Tauri 내부의 숨은 창 포함)에
`WM_CLOSE` 를 보내므로 이벤트 루프를 깨뜨린 것으로 본다(Tauri 내부까지 확인하지는 않았다).

반대로 `WM_CLOSE` 를 제목이 "AgentsAssemble" 인 보이는 메인 창 **하나에만** 보내면(X 버튼과 같음)
세 번 연속 0.3 초에 데스크톱 · 감독 · 서버가 모두 끝났다(이미지 붙여넣기 뒤 닫기 포함). 사용자가 X
버튼으로 닫을 때 이 문제가 나는지는 따로 확인하지 않았다. 자동화에서는 메인 창에만 `WM_CLOSE` 를
보내고, `taskkill` 과 강제 종료는 쓰지 않는다. 빌드가 실행 파일 잠금으로 실패하면 옛 실행 파일을 다시
띄우지 말 것(두 번째 인스턴스의 WebView 가 첫 인스턴스 브라우저 프로세스에 붙는다).
