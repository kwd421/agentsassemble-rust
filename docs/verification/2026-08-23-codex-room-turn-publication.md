...
- Post-fix `make verify`: passed, including architecture and 800-line source gates, generated TypeScript, React production build and 29 frontend tests, Tauri checks and 12 tests, 54 persistence tests, 57 provider tests, 9 server unit tests, 14 server boundary/integration tests, warning-denied workspace Clippy, doc tests, and diff whitespace checks.
- Manual security review found that authenticated messages could grow a busy session's durable pending/inflight event-ID vectors without a hard aggregate cap and that restart merging used quadratic duplicate checks. The fix caps the combined queue at 256 IDs inside the source-message transaction, rejects overflow without a visible message or command result, fails closed on oversized/noncanonical stored authority, and uses one shared bounded `HashSet` merge for failure, stop, and restart paths. Focused boundary tests cover atomic overflow rollback and bounded deduplication.
