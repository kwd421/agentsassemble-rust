...
The common provider adapter owns live runtime slots and one supervisor identity independently of room persistence. A driver may know Codex JSONL, Antigravity PTY/ConPTY, or OpenCode HTTP/SSE, but it does not decide room lifecycle, replay, publication, or recovery semantics. The Codex driver binds the verified executable object through process creation: Linux and Android copy the verified bytes into a sealed executable `memfd`, other Unix targets execute a byte-verified `0500` copy held inside an explicitly verified private `0700` staging directory, and Windows holds the verified image open without write/delete sharing. Staged bytes are hashed directly with the already-open source object's stable identity. Linux/Android bind the running server through `/proc/self/exe`; on macOS the desktop supervisor first copies and verifies the server into a private `0700` staging directory, and provider custody is unavailable if that launch proof is absent. The guardian then binds that exact running server object before either helper re-executes. Codex uses a credential-free environment allowlist, `app-server --stdio`, process-local model/effort/tier/sandbox/approval/workspace-trust configuration, one bounded JSONL reader, a 256-message/2 MiB aggregate pre-turn notification queue, and default-denied server requests.

Every current-profile runtime acquires a generation-tokened exact room/session lease before the provider can spawn. Windows keeps that lock in the supervisor. Unix uses an internal guardian and a stable process-group anchor. The anchor atomically records its process-group ID while taking the lease; the provider joins its group and inherits the unguessable lease-generation tag. Guardian shutdown freezes the anchored group before enumerating that tag. Linux signals escaped processes only through revalidated pidfds and then kills the group; macOS and Android never signal a group-external numeric PID and instead refuse to confirm shutdown. Guardian or server death follows the same cleanup path. An unlocked lease marker proves `Gone` only when both its recorded group and its tagged generation are absent, so an escaped orphan keeps recovery locked rather than permitting a duplicate runtime. Confirmed normal-shutdown observations retain the exact lease generation until their absence is durably checkpointed, then remove only that generation. Once process creation succeeds, cancellation cannot discard custody and initialization failure retains exact uncertain authority until shutdown is confirmed. Process start and provider thread attachment remain separate checkpoints.

Executable binding protects selection against path, symlink, updater, in-place-write, and atomic-replacement races within the application's trusted local account. A hostile process already executing as the same OS account is outside this boundary: on Unix it can generally interfere with peer processes and account-private files, and macOS exposes no sealed descriptor-execution path equivalent to Linux `memfd`. Network peers, room inputs, and provider output remain untrusted.
