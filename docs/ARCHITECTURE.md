...
Startup reconciliation is a three-owner protocol: persistence loads a complete private candidate outside process I/O, the common provider supervisor reports only `Adopted`, `Gone`, or `Ambiguous`, and persistence validates and commits the lifecycle-specific transition with an exact candidate CAS. Drivers report facts and never choose `owner_lost`. A gone pending stop becomes `effect_applied`; a confirmed checkpoint never repeats an effect; an adoptable runtime is rebound to the current supervisor; ambiguous or foreign lifecycle ownership becomes terminal `owner_lost`. Network admission occurs only after every loaded candidate has committed a current observation.

The common provider adapter owns live runtime slots and one supervisor identity independently of room persistence. A driver may know Codex JSONL, Antigravity PTY/ConPTY, or OpenCode HTTP/SSE, but it does not decide room lifecycle, replay, publication, or recovery semantics. The Codex driver uses the selected exact executable, a credential-free environment allowlist, `app-server --stdio`, process-local model/effort/tier/sandbox/approval/workspace-trust configuration, one bounded JSONL reader, default-denied server requests, and an owned process group or Job Object. Process start and provider thread attachment remain separate checkpoints.
