...
The common provider adapter owns live runtime slots and one supervisor identity independently of room persistence. A driver may know Codex JSONL, Antigravity PTY/ConPTY, or OpenCode HTTP/SSE, but it does not decide room lifecycle, replay, publication, or recovery semantics. The Codex driver binds the verified executable object through process creation: Linux and Android copy the verified bytes into a sealed executable `memfd`, other Unix targets execute a byte-verified `0500` copy held inside a private `0700` staging directory, and Windows holds the verified image open without write/delete sharing. It then uses a credential-free environment allowlist, `app-server --stdio`, process-local model/effort/tier/sandbox/approval/workspace-trust configuration, one bounded JSONL reader, a 256-message/2 MiB aggregate pre-turn notification queue, and default-denied server requests.

Every current-profile runtime acquires an exact room/session lease before the provider can spawn. Windows keeps that lock in the supervisor. Unix uses an internal guardian and a stable process-group anchor: the anchor owns the lease and group identity, the provider joins its group, guardian or server death closes the anchor control pipe, and shutdown kills the anchored group before reaping the exact anchor. A leader exit is therefore only an uncertain health fact, never proof that the runtime tree is gone, and a fresh supervisor may report `Gone` only from the exact unlocked lease. Confirmed normal-shutdown observations are durably checkpointed before their lease files are released. Once process creation succeeds, cancellation cannot discard custody and initialization failure retains exact uncertain authority until shutdown is confirmed. Process start and provider thread attachment remain separate checkpoints.
