...
The 2026-09-07 user scope decision excludes Freebuff and retires every managed
Antigravity catalog, process, PTY/ConPTY, hook, and native-receipt path. Historical
copies are outside the build and runtime graph. Antigravity is solely a Phase 7
externally launched CLI using a Room Connector invitation; it has no managed Agent
Session controls and does not use human admission. The retained managed inventory
and current phase gates are owned by `docs/PRODUCT_REIMPLEMENTATION_PLAN.md`.
Historical verification below cannot reinstate a retired provider contract.

The defensive-complexity audit at Rust baseline `8a5f75a` also rejects a security label as
...
challenge and startup-secret preamble had no production caller and were removed by
D-01 at `a7949bd`; they are not part of the credential contract. The private control pipe
...
lifecycle from profile, preference, pin, search, message-attachment, and bound
room-appearance operations. Each corrected target route authenticates the
...
Unix runtime handles and activated lease markers bind the exact current OS boot identity and the same launch-generation token. Linux and Android use the kernel boot UUID; macOS reads `kern.bootsessionuuid` through the maintained safe `sysctl` crate, parses it as a UUID, canonicalizes its representation, and never uses mutable wall-clock or boot-epoch arithmetic. The private boot value is platform-domain-separated and hashed before storage, cached once per server process, and never enters public state. `runtime_handle.rs` is the one strict runtime-v6 decoder for Unix and Windows, while `runtime_absence.rs` is the one owner that may turn a lease observation into `Gone`. Runtime-v6 removes the former parsed-but-unused random UUID; the separately stored canonical launch token remains the sole lease-generation identity. Cold Unix recovery requires the handle, durable token, and available marker witnesses to agree on boot and launch generation; a missing lease additionally requires an old-boot handle. Cold Windows recovery requires the Windows handle, durable token, and generation-gone marker to carry the same launch token. A live in-memory slot never accepts `PreviousBoot`: that observation contradicts the current process's launch generation and remains fail-closed. `Unknown`, a current-boot handle paired with an old marker, a substituted token, a malformed value, a source error, a cross-platform handle, or any other mismatch remains `Ambiguous` and retains the exact request, intent, handle, owner, and token. The runtime-handle format and required durable representation changed together, so current schema 57 rejects schema 56 and every other non-current schema without conversion, compatibility, or fallback behavior.
...
Cursor uses one persistent `cursor-agent acp` child through the maintained official ACP client library and protocol V1. A bounded `cursor/list_available_models` request over the native ACP connection owns the live catalog; the Cursor module groups only exact advertised model/effort/service-tier tuples, while the common catalog and selection owners enforce the selected tuple. The ACP session owner requires native HTTP MCP support, creates or loads exactly one durable provider session, confirms the exact selected model through the session configuration contract, and accepts output only for that attached session and active turn. Protocol lines are capped at 256 KiB and accumulated assistant output at 128 KiB. An authorized interrupt sends the native session cancel notification and retains the runtime only after the matching prompt ends with `cancelled` within ten seconds; protocol ambiguity poisons the driver and requires restart. Normal long turns have no invented total deadline.

The Cursor process owner supplies one private bearer-authenticated RoomPortal as the ACP HTTP MCP server, retains executable/process/stderr/protocol/portal custody together, and reuses the common room-observation finalizer so ACP assistant text alone never becomes canonical room authority. All ACP permission requests currently reject or cancel: the installed protocol does not yet prove a narrower tool-specific approval boundary suitable for workspace write, so Cursor advertises only `meeting_read_only`. That limitation is explicit until phase-wide authorized-tool hardening and a real Cursor flow prove more; it is not filled by terminal output, transcript parsing, a second corrective prompt, polling, retry, or fallback.
