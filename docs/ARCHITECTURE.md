...
`domain` cannot depend on Tokio, Axum, SQLx, Tower, or provider/network mechanisms. `scripts/check_architecture.py` rejects unowned workspace crates, dependency-direction violations, and every local path dependency outside the explicit owned-crate allowlist. `scripts/check_source_growth.py` applies responsibility-review warnings at 500 and 800 lines and an absolute 1,000-line ceiling to handwritten and generated source alike, so a filename cannot bypass the gate. These thresholds never authorize mixed responsibilities: a small file still splits when concepts or authorities differ, while a larger file remains intact only when one cohesive flow and invariant genuinely require it.
