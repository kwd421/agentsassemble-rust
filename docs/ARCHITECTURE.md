...
The watcher scans at most 64 Agent Session keys per page, observes at most eight candidates concurrently, and bounds each observation to two seconds. Startup no longer performs a duplicate full reconciliation pass. These limits bound CPU, memory, SQLite work, and shutdown latency without introducing a cache or second authority.
