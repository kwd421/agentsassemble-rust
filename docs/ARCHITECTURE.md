...
Every current-profile runtime acquires a generation-tokened exact room/session lease before the provider can spawn. Windows keeps that lock in the supervisor. Unix uses an internal guardian and a stable process-group anchor plus a separate lifetime lease. The anchor atomically records its process-group ID while taking the main lease; the provider joins that group and inherits both an unguessable generation tag and a shared lock descriptor for the exact lifetime-file inode. The guardian runs in a process group independent of the server, so desktop/server group termination leaves its cleanup path alive; the outer desktop grace covers the nested server and provider shutdown budgets. Guardian shutdown freezes the anchored group before inspecting escaped ownership. Linux scans same-user processes for the inherited lifetime inode or the auxiliary tag, revalidates PID/start identity around pidfd acquisition, and signals group-external matches only through pidfds. Android detects lifetime-inode or tag matches but refuses unsafe numeric signaling; macOS uses the tag for pre-kill detection and the lifetime lock as the exact postcondition, likewise failing closed for a group-external process. A server or desktop loss closes the guardian control pipe and enters that cleanup path. An unlocked main marker proves `Gone` only when its recorded group and tag are absent and the exact lifetime lock is no longer held, so clearing the environment and changing session/process group cannot create a false absence proof. Confirmed normal-shutdown observations retain the exact lease generation until their absence is durably checkpointed, then remove only that generation. Once process creation succeeds, cancellation cannot discard custody and initialization failure retains exact uncertain authority until shutdown is confirmed. Process start and provider thread attachment remain separate checkpoints.

Executable binding protects selection against path, symlink, updater, in-place-write, and atomic-replacement races within the application's trusted local account. A hostile process already executing as the same OS account, or a deliberately hostile selected provider that strips both internal custody signals from a descendant, is outside this boundary: on Unix it can generally interfere with peer processes and account-private files, and macOS exposes neither a sealed descriptor-execution path equivalent to Linux `memfd` nor a pidfd-style stable signaling handle. Network peers, room inputs, and provider output remain untrusted.
