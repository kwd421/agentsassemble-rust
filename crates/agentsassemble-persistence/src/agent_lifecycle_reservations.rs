...
pub(crate) async fn claim_lifecycle_command(
    ...
    let existing = sqlx::query(
        "SELECT action, payload_hash, session_id, operation_id FROM lifecycle_command_reservations WHERE room_id = ? AND principal_id = ? AND request_id = ?",
    )
    ...
    sqlx::query(
        "INSERT INTO lifecycle_command_reservations(room_id, principal_id, request_id, action, payload_hash, session_id, operation_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
    )
    .bind(&reservation.principal.room_id)
    .bind(&reservation.principal.principal_id)
    .bind(reservation.request_id)
...
pub(crate) async fn finish_lifecycle_command(
    ...
    let removed = sqlx::query(
        "DELETE FROM lifecycle_command_reservations WHERE room_id = ? AND principal_id = ? AND request_id = ? AND action = ? AND payload_hash = ? AND session_id = ? AND operation_id = ?",
    )
    .bind(&reservation.principal.room_id)
    .bind(&reservation.principal.principal_id)
    .bind(reservation.request_id)
