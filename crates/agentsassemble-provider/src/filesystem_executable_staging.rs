...
const ROOT_NAME: &str = "agentsassemble-provider-executable-staging-v1";
...
pub(super) struct ExecutableStaging {
    directory: tempfile::TempDir,
    _lease: File,
}

impl ExecutableStaging {
...
fn ensure_private_directory(path: &Path) -> io::Result<()> {
        ...
        || metadata.permissions().mode() & 0o077 != 0
    {
        return Err(io::Error::other(
            "executable staging root is not a private owned directory",
        ));
    }
    Ok(())
}
...
fn acquire_lock(path: &Path, create: bool, wait: bool) -> io::Result<File> {
    ...
    {
        return Err(io::Error::other(
            "executable staging lock is not a private owned file",
        ));
    }
    if wait {
...
fn reclaim_stale_directories(root: &Path) -> io::Result<()> {
    ...
    for entry in fs::read_dir(root)? {
        ...
        if candidates.len() == MAX_STAGING_DIRECTORIES {
            return Err(io::Error::other(
                "executable staging directory limit was exceeded",
            ));
        }
...
fn reclaim_stale_directory(path: &Path) -> io::Result<()> {
    let name = path
        ...
        .ok_or_else(|| io::Error::other("executable staging name is invalid"))?;
    if !name.starts_with(DIRECTORY_PREFIX) {
        return Err(io::Error::other(
            "executable staging root contains an unknown entry",
        ));
    }
    let metadata = fs::symlink_metadata(path)?;
    if !metadata.is_dir()
    ...
    {
        return Err(io::Error::other(
            "executable staging entry is not a private owned directory",
        ));
    }
...
mod tests {
    ...
    fn next_owner_reclaims_only_unlocked_staging() {
        let base =
            tempfile::tempdir().unwrap_or_else(|error| panic!("create staging test root: {error}"));
        let root = base.path().join("staging");
        let active = ExecutableStaging::create_in(&root)
            .unwrap_or_else(|error| panic!("create active staging: {error}"));
        let active_path = active.path().to_path_buf();

        let root_lock = acquire_lock(&root.join(ROOT_LOCK_NAME), false, true)
            .unwrap_or_else(|error| panic!("lock staging root: {error}"));
        let abandoned = tempfile::Builder::new()
            .prefix(DIRECTORY_PREFIX)
            .permissions(fs::Permissions::from_mode(0o700))
            .tempdir_in(&root)
            .unwrap_or_else(|error| panic!("create abandoned staging: {error}"));
        let abandoned_path = abandoned.keep();
        drop(root_lock);

        let replacement = ExecutableStaging::create_in(&root)
            .unwrap_or_else(|error| panic!("create replacement staging: {error}"));
