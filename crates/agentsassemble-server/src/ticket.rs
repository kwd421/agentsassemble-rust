...
pub struct TicketStore {
    grants: Arc<Mutex<HashMap<String, TicketGrant>>>,
    ttl: Duration,
    capacity: usize,
}
...
impl TicketStore {
    ...
    pub async fn issue(&self, principal: AuthenticatedPrincipal) -> Result<String, TicketError> {
        let now = Instant::now();
    ...
    pub async fn consume(&self, ticket: &str) -> Result<AuthenticatedPrincipal, TicketError> {
        let grant = self
            .grants
            .lock()
            .await
...
mod tests {
    ...
    async fn ticket_is_consumed_once() {
        ...
        assert!(store.consume(&ticket).await.is_ok());
        assert_eq!(store.consume(&ticket).await, Err(TicketError::Invalid));
    }
