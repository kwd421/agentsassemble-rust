...
/// Issues an exact attendee-invite-create credential for the current local room manager.
///
/// # Errors
///
/// Returns a bounded room, manager, persistence, or ticket-capacity error.
pub async fn issue_attendee_invite_create_ticket(
    state: &AppState,
    ...
    let issued = state
        .tickets
        .issue_attendee_invite_create(authority)
        .await
        .map_err(|_| TicketIssueError::Unavailable)?;
    Ok(operator_http_response(state, issued))
}
