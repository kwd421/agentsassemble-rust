...
- Active task: F-11 frontend wire-contract generation ownership.
  ...
  - Original defect: the copied `generatedRoomEvent.ts`, handwritten provider/snapshot
    ...
    accepts them.
  - Current batch: the Participant half is implemented in local commits `4fc06a0`,
    `2f8ebbb`, and `c40fe55`, pending pushed manual cross-review. Role mutation ACKs now
  ...
  - Participant-batch verification: the three commits are independently buildable and
    ...
    remaining work is to push and manually cross-review this exact batch; F-11 stays open
    until both reviewers approve it.
