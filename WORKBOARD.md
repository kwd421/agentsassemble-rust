...
Status: Phase 5 — the admitted-human WebSocket exchange, exact session lifetime, bounded command provenance, copied frontend transport, reachable one-use/reusable normal/read-only browser matrix, and remote human preferences are implemented and verified. `participant.leave`, manager invite controls, and trusted public ingress stay explicitly incomplete.
...
- Comparison baseline: original `d5046473010d1353a81ee38337360e6d98f7bd6f`; public Rust baseline `80243e8`.
- Required order: connect each remaining typed exchange only with its exact target revalidation, beginning with `participant.leave` and then manager invite controls. Follow with trusted public ingress and appearance. Update the exposure map only after a surface is reachable and verified.
