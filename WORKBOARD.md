...
- Comparison baseline: original `d5046473010d1353a81ee38337360e6d98f7bd6f`; public Rust baseline `f608422`.
- Required order: follow the active owner's approved Stage B authority and activation boundaries; update the exposure map only after a surface is reachable and verified; inspect `git diff --stat`, then commit and push each independently buildable, verifiable, and rollbackable change before both manual reviews.
