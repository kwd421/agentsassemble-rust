...
- Active gate: the appearance persistence and local/remote HTTP batch plus its review corrections are fully verified, pushed, and manually approved through `d82b8e2`; the local feature queue is empty.
...
- Required order: implement and package-verify typed desktop issuance plus the copied frontend object-URL lifecycle. Update the exposure map only after that surface is reachable and verified.
