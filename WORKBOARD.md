...
- Active task: F-11 frontend wire-contract generation ownership.
  ...
  - Original defect: the copied `generatedRoomEvent.ts`, handwritten provider/snapshot
    ...
    accepts them.
  - Remaining boundary: the Participant half still owns legacy `RoomMember` fields and
    the producerless member `thinking` signal. It remains separate from the completed
    Agent Session identity/lifecycle half because room role, join state, mute state, and
    membership belong to the generated Participant owner.
