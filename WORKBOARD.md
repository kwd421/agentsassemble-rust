...
- Comparison baseline: original `d5046473010d1353a81ee38337360e6d98f7bd6f`; public Rust baseline `3dbeceb`.
- Required order: design and implement the complete human invite/admission/session owner and live-session-bound one-use preference exchanges before remote preference or appearance activation; update the exposure map only after a surface is reachable and verified; inspect `git diff --stat`, then commit and push each independently buildable, verifiable, and rollbackable change before both manual reviews.
