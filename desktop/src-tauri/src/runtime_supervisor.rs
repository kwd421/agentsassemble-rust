...
) -> Result<RuntimeSupervisorCommand, String> {
    let helper = desktop.with_file_name(format!(
        "agentsassemble-runtime-supervisor{}",
        std::env::consts::EXE_SUFFIX
    ));
    #[cfg(target_os = "macos")]
    let binding = BoundSidecar::bind(&helper)
        .map_err(|error| format!("cannot bind standalone runtime supervisor: {error}"))?;
    #[cfg(target_os = "macos")]
...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn signed_bundle_uses_a_valid_standalone_supervisor() {
        let directory = tempfile::tempdir()
            .unwrap_or_else(|error| panic!("create signed bundle fixture: {error}"));
        let bundle = directory.path().join("Fixture.app");
        let binaries = bundle.join("Contents/MacOS");
        std::fs::create_dir_all(&binaries)
            .unwrap_or_else(|error| panic!("create fixture bundle: {error}"));
        let main = binaries.join("fixture");
        let helper = binaries.join("agentsassemble-runtime-supervisor");
        let current = std::env::current_exe()
            .unwrap_or_else(|error| panic!("resolve test executable: {error}"));
        for target in [&main, &helper] {
        ...
        std::fs::write(
            ...
            "<?xml version=\"1.0\" encoding=\"UTF-8\"?><plist version=\"1.0\"><dict>\
             <key>CFBundleIdentifier</key><string>app.agentsassemble.signing-fixture</string>\
             <key>CFBundleExecutable</key><string>fixture</string>\
             <key>CFBundlePackageType</key><string>APPL</string></dict></plist>",
        )
        .unwrap_or_else(|error| panic!("write fixture bundle metadata: {error}"));
        for target in [&helper, &bundle] {
            let output = Command::new("/usr/bin/codesign")
                .args(["--force", "--sign", "-"])
                .arg(target)
                .output()
                .unwrap_or_else(|error| panic!("sign local fixture: {error}"));
            assert!(
        ...
        let invalid_app_copy =
            BoundSidecar::bind(&main).unwrap_or_else(|error| panic!("bind app fixture: {error}"));
        let verify = |path: &std::path::Path| {
            Command::new("/usr/bin/codesign")
                .args(["--verify", "--strict"])
                .arg(path)
                .output()
                .unwrap_or_else(|error| panic!("verify fixture signature: {error}"))
        };
        assert!(!verify(invalid_app_copy.launch_path()).status.success());
        let selected = command_for_desktop(&helper, &main)
            .unwrap_or_else(|error| panic!("select signed supervisor: {error}"));
        let selected_path = std::path::Path::new(selected.command.get_program());
        let verification = verify(selected_path);
        assert!(
            verification.status.success(),
            "{}",
            String::from_utf8_lossy(&verification.stderr)
        );
        let output = Command::new(selected_path)
            .arg("--list")
            .output()
            .unwrap_or_else(|error| panic!("execute signed standalone supervisor: {error}"));
