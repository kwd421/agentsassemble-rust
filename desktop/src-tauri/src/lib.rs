...
#[tauri::command(rename_all = "camelCase")]
async fn runtime_message_search_read_ticket(
    window: WebviewWindow,
    app: tauri::AppHandle,
    room_id: String,
) -> Result<HttpTicketGrant, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        runtime_app
            .state::<LocalRuntime>()
            .issue_message_search_read_ticket(&runtime_app, &room_id)
    })
    .await
    .map_err(|error| format!("runtime message-search read ticket worker failed: {error}"))?
}

#[tauri::command(rename_all = "camelCase")]
async fn runtime_side_chat_read_ticket(
    window: WebviewWindow,
    app: tauri::AppHandle,
    room_id: String,
) -> Result<HttpTicketGrant, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        runtime_app
            .state::<LocalRuntime>()
            .issue_side_chat_read_ticket(&runtime_app, &room_id)
    })
    .await
    .map_err(|error| format!("runtime side-chat read ticket worker failed: {error}"))?
}

#[tauri::command(rename_all = "camelCase")]
async fn runtime_message_attachment_upload_ticket(
    window: WebviewWindow,
    app: tauri::AppHandle,
    room_id: String,
) -> Result<HttpTicketGrant, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        runtime_app
            .state::<LocalRuntime>()
            .issue_message_attachment_upload_ticket(&runtime_app, &room_id)
    })
    .await
    .map_err(|error| format!("runtime message-attachment upload ticket worker failed: {error}"))?
}

#[tauri::command(rename_all = "camelCase")]
async fn runtime_message_attachment_read_ticket(
    window: WebviewWindow,
    app: tauri::AppHandle,
    room_id: String,
    ...
    tauri::async_runtime::spawn_blocking(move || {
        runtime_app
            .state::<LocalRuntime>()
            .issue_message_attachment_read_ticket(&runtime_app, &room_id, &attachment_id)
    })
    .await
    .map_err(|error| format!("runtime message-attachment read ticket worker failed: {error}"))?
}

#[tauri::command(rename_all = "camelCase")]
async fn runtime_attendee_invite_create_ticket(
    window: WebviewWindow,
    app: tauri::AppHandle,
    ...
    tauri::async_runtime::spawn_blocking(move || {
        runtime_app
            .state::<LocalRuntime>()
            .issue_attendee_invite_create_ticket(&runtime_app, authority)
    })
    .await
    .map_err(|error| format!("runtime attendee invite create ticket worker failed: {error}"))?
}
...
async fn runtime_appearance_upload_ticket(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        runtime_app
            .state::<LocalRuntime>()
            .issue_appearance_upload_ticket(&runtime_app, authority)
    })
    .await
    .map_err(|error| format!("runtime appearance upload ticket worker failed: {error}"))?
}

#[tauri::command(rename_all = "camelCase")]
async fn runtime_appearance_pending_read_ticket(
    window: WebviewWindow,
    app: tauri::AppHandle,
    authority: ManagerRoomAuthority,
    asset_id: String,
    ...
    tauri::async_runtime::spawn_blocking(move || {
        runtime_app
            .state::<LocalRuntime>()
            .issue_appearance_pending_read_ticket(&runtime_app, authority, &asset_id)
    })
    .await
    .map_err(|error| format!("runtime appearance pending-read ticket worker failed: {error}"))?
}

#[tauri::command(rename_all = "camelCase")]
async fn runtime_appearance_bound_read_ticket(
    window: WebviewWindow,
    app: tauri::AppHandle,
    authority: ManagerRoomAuthority,
    asset_id: String,
    ...
    tauri::async_runtime::spawn_blocking(move || {
        runtime_app
            .state::<LocalRuntime>()
            .issue_appearance_bound_read_ticket(&runtime_app, authority, &asset_id)
    })
    .await
    .map_err(|error| format!("runtime appearance bound-read ticket worker failed: {error}"))?
}
